...
class ModelBaseline(ModelBase):
    ...
    def __init__(self, opt):
        super(ModelBaseline, self).__init__(opt)
        # ------------------------------------
        # define network
        # ------------------------------------
    ...
    def define_loss(self):
        ...
        # mali define insensitivity loss function
        IS_lossfn_type = self.opt_train['IS_lossfn_type']
        if IS_lossfn_type == 'l1':
            self.IS_lossfn = nn.L1Loss().to(self.device)
        elif IS_lossfn_type == 'l2':
            self.IS_lossfn = nn.MSELoss().to(self.device)
        elif IS_lossfn_type == 'l2sum':
            self.IS_lossfn = nn.MSELoss(reduction='sum').to(self.device)
        else:
            raise NotImplementedError('Loss type [{:s}] is not found.'.format(QF_lossfn_type))
