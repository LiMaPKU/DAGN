...
class Discriminator_VGG_8(nn.Module):
    def __init__(self, in_nc=512, base_nc=512, ac_type='BL', pooling=False):
        super(Discriminator_VGG_8, self).__init__()
        # features
        # hxw, c
        # 8, 5121
        conv0 = B.conv(in_nc, base_nc, kernel_size=3, mode='C')
        conv1 = B.conv(base_nc, base_nc//4, kernel_size=3, mode='C'+ac_type)
        conv2 = B.conv(base_nc//4, base_nc//16, kernel_size=3, mode='C'+ac_type)

        self.features = B.sequential(conv0, conv1, conv2)

        # classifier
        self.classifier = nn.Sequential(
            nn.Linear(base_nc//16 * 12 * 12, 100), nn.LeakyReLU(0.2, True), nn.Linear(100, 1), nn.Sigmoid())
